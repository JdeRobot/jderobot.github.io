...
- **Brief explanation**: Currently, the RADI is based on Gazebo11 version, which its end of life (in Sep, 2025) is getting closer. The main goal of the project is to migrate the RADI to Gazebo Fortress. This migration will take form by having a couple of exercises running in ROS2 with the new RADI.
...
- **Expected size**: 175h
- **Mentors**: Pedro Arias (pedro.ariasp AT upm.es) and Pawan

## Project #2: CI & Testing

- **Brief explanation**: Improve maintainability on main software products of RoboticsAcademy via testing and continuous integration.
...
- **Expected results**: 
- **Expected size**: 
- **Mentors**: David Pascual

## Project #4: Robotics-Academy: new exercise using Deep Learning for Visual Control 

Artificial Intelligence based exercises at RoboticsAcademy
...
And real robotics applications.
...
LIDAR sensor in autoparking. Crossing overcome
...
## Project #10: End-to-end autonomous vehicle control based on text-based instructions: research project regarding Autonomous Driving + LLMs

- **Brief explanation**: In previous editions of GSoC, we have explored Behavior Metrics as a tool for evaluation of end-to-end autonomous driving models, we have also explored optimization of end-to-end models and even navigation based on segmented images and commands. 
In this project, we would like to combine the previous acquired knowledge with LLMs, using the projects the we previously developed successfully on GSoC. The idea is to integrate an LLM system with an end-to-end autonomous driving control model so the user can given commands in text mode directly to the vehicle, as you’d do when using a real-life taxi (“please, stop here”, “please, turn to the left in the following intersection”). This idea has been already explored in research projects and we will be replicating it.
